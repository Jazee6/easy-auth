...
export const oauthClientAudit = sqliteTable(
  "oauth_client_audit",
  {
    id: text("id").primaryKey(),
    ...
    action: text("action", {
      enum: ["create", "update", "disable", "enable", "rotate-secret", "delete"],
    }).notNull(),
  ...
  (table) => [
    index("oauthClientAudit_ownerUserId_createdAt_idx").on(table.ownerUserId, table.createdAt),
    index("oauthClientAudit_ownerUserId_clientId_idx").on(table.ownerUserId, table.clientId),
  ],
);
